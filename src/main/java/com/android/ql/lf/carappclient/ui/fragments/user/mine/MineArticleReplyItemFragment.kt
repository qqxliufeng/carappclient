...
import com.android.ql.lf.carappclient.ui.fragments.community.ArticleInfoFragment
...
class MineArticleReplyItemFragment : BaseRecyclerViewFragment<ArticleAnswerBean>() {
    ...
    override fun onMyItemClick(adapter: BaseQuickAdapter<*, *>?, view: View?, position: Int) {
        ...
        article.merchant_quiz_id = mArrayList[position].merchant_answer_qid
    ...
    override fun onMyItemChildClick(adapter: BaseQuickAdapter<*, *>?, view: View?, position: Int) {
        if (view!!.id == R.id.mBtArticleDelete) {
            ...
            build.setPositiveButton("删除") { _, _ ->
                mPresent.getDataByPost(0x1, RequestParamsHelper.QAA_MODEL, RequestParamsHelper.ACT_DEL_QAA, RequestParamsHelper.getDelQaaParam(aid = mArrayList[position].merchant_answer_id))
            }
    ...
    class MineReplyArticleListAdapter(resId: Int, list: ArrayList<ArticleAnswerBean>) : BaseQuickAdapter<ArticleAnswerBean, BaseViewHolder>(resId, list) {
        override fun convert(helper: BaseViewHolder?, item: ArticleAnswerBean?) {
            ...
            helper.setText(R.id.mTvMyArticleAnswerItemInfoTitle, item!!.merchant_answer_title)
            helper.setText(R.id.mTvMyArticleAnswerItemInfoContent, "回复：${item.merchant_answer_content}")
        }
    }
}