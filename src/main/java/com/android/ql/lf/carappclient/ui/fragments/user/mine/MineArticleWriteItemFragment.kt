...
import com.android.ql.lf.carappclient.ui.fragments.community.WriteArticleFragment
...
class MineArticleWriteItemFragment : BaseRecyclerViewFragment<ArticleBean>() {
    ...
    override fun onMyItemChildClick(adapter: BaseQuickAdapter<*, *>?, view: View?, position: Int) {
        if (view!!.id == R.id.mBtArticleDelete) {
            ...
            build.setPositiveButton("是") { _, _ ->
                mPresent.getDataByPost(0x1, RequestParamsHelper.QAA_MODEL, RequestParamsHelper.ACT_DEL_QAA, RequestParamsHelper.getDelQaaParam(qid = mArrayList[position].merchant_quiz_id))
            }
    ...
    class MineWriteArticleListAdapter(resId: Int, list: ArrayList<ArticleBean>) : BaseQuickAdapter<ArticleBean, BaseViewHolder>(resId, list) {
        override fun convert(helper: BaseViewHolder?, item: ArticleBean?) {
            ...
            helper.setText(R.id.mTvMyArticleItemInfoTitle, item!!.merchant_quiz_title)
            helper.setText(R.id.mTvMyArticleItemInfoContent, "内容：${item.merchant_quiz_content}")
            helper.setText(R.id.mTvMyArticleItemInfoReplyCount, "${item.merchant_quiz_replies}回复")
            helper.setText(R.id.mTvMyArticleItemInfoPraiseCount, "${item.merchant_quiz_click}赞")
        }
    }
